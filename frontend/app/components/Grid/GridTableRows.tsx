...
const MemoizedRowRenderer = React.memo(
  ({
    row,
    index,
    style,
  ...
  (prev, next) => {
    // Memoize only if neither the focused row nor edited row changed
    return (
      prev.index === next.index &&
      prev.focusedCell?.rowIndex !== next.index &&
      prev.editingCell?.rowIndex !== next.index
    );
  }
);
